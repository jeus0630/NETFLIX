import * as React from 'react';
import WidgetLg from '../widgetLg/WidgetLg';
import "./widgetSm.scss";
import { Visibility } from '@mui/icons-material';
import { useDispatch, useSelector } from "react-redux";
import { AppDispatch, RootState } from '../../redux/store';
import { getNewUsers } from '../../redux/statSlice';
import { useEffect } from "react";

interface IWidgetSmProps {
}

const WidgetSm: React.FunctionComponent<IWidgetSmProps> = (props) => {

  const dispatch = useDispatch<AppDispatch>();
  const newUsers = useSelector((state: RootState) => state.stats.newUsers);

  useEffect(() => {

    dispatch(getNewUsers());

    return () => {

    }
  }, [])


  return (
    <div className="widget-sm">
      <h3 className="widget-sm-title">
        New Join Members
      </h3>
      <ul className="widget-sm-list">
        {
          newUsers.map(newUser => {
            return (
              <li key={newUser._id} className="widget-sm-list-item">
                <img src={newUser.profilePic || "data:image/png;base64,"} alt="" className="widget-sm-img" />
                <div className="widget-sm-user">
                  <span className="widget-sm-user-name">{newUser.username}</span>
                  <span className="widget-sm-user-title">created at: {newUser.createdAt.split('T')[0]}</span>
                </div>
                <button className="widget-sm-button">
                  <Visibility className="widget-sm-icon"></Visibility>
                  Display
                </button>
              </li>
            )
          })
        }
      </ul>
    </div>
  );
};

export default WidgetSm;
